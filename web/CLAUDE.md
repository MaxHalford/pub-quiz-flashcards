...
- **Spaced repetition via FSRS, opinionated.** Two buttons map to `Again` / `Good`; we deliberately throw away the `Hard`/`Easy` signal. FSRS' short-term (Learning/Relearning) steps are disabled (`enable_short_term: false` in `scheduler.ts`) so every rating schedules at least a day ahead — a correct answer never resurfaces a card the same day or the next. No user-facing toggle for this.
